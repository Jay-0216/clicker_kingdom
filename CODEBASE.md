...
- **CPS 표시 버그 수정 (`js/game-config.js`, `index.html`, `js/app.js`)**:
  - `formatNumber()`가 1000 미만 값을 항상 `Math.floor()`로 정수화해서, CPS가 0.1~0.9처럼 소수인 경우 화면에 "+0 /초"로 표시되던 버그. 정수가 아닌 경우 소수점(`decimals`)까지 표시하도록 수정.
  - 메인 클릭 화면 상단 라벨의 "자동 수확"을 "CPS"로 표기 통일 (`cpsLabel`).
