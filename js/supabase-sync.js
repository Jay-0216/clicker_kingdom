...
function safeSyncString(val, defaultValue = '0') {
  if (typeof val === 'string') return val;
  if (typeof val === 'bigint') return val.toString();
  if (typeof val === 'number') {
    if (!isFinite(val)) return defaultValue;
...
function updateCloudSyncState(patch) {
  ...
  if (patch.tone === 'error' || patch.tone === 'warning') {
    reportCloudErrorToAdmin(patch);
  }
}
  ...
  try {
    const { error } = await client
      ...
      .upsert({
        ...
        clicks: safeSyncString(account.clicks, "0"),
