...
function renderQuickUpgrades(clicks) {
  ...
  const affordableRelics = MULTIPLIER_RELICS.filter(r => {
    return clicks >= r.cost && !state.relics[r.id];
  });
    ...
    html += `
      <div class="quick-upgrade-card">
        <div class="quick-upgrade-info">
          ...
          <div>
            <div class="quick-upgrade-name">${topRelic.name}</div>
            <div class="quick-upgrade-sub">${topRelic.desc}</div>
          </div>
        </div>
        <button class="buy-btn" data-quick-buy-relic="${topRelic.id}">구매 (${topRelic.cost.toLocaleString()})</button>
      </div>
    `;
  });
