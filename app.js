  ...
  if (state.currentUser) {
    actions.innerHTML = `
      <div class="user-chip-wrap" id="userChipWrap">
        <button class="user-chip" id="userChip" type="button">${escapeHtml(state.currentUser.nickname)} ▾</button>
        <div class="user-chip-dropdown" id="userChipDropdown" hidden>
          <div class="user-chip-dropdown-label">아이디</div>
          <div class="user-chip-dropdown-id">${escapeHtml(state.currentUser.id)}</div>
        </div>
      </div>
    ...
    // 닉네임 클릭 시 아이디 드롭다운 토글
    const chipBtn = document.getElementById('userChip');
    ...
    chipBtn.onclick = (e) => {
      ...
      if (willOpen) {
        // 드롭다운이 열릴 때만 바깥 클릭 감지 리스너를 1회성으로 등록 (중복 등록 방지)
        setTimeout(() => {
